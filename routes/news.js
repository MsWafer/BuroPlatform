...
router.get('/all',auth,async(req,res)=>{
    try {
        let news = await News.find().sort({postDate:-1}).populate('author','-password -permission -projects -tickets')
...
router.get('/:id',auth,async(req,res)=>{
    try {
        let news = await News.findOne({_id:req.params.id}).populate('author','-password -permission -projects -tickets')
...
router.put('/:id',auth,async(req,res)=>{
    try {
        ...
        let news = await News.findOneAndUpdate({_id:req.params.id},{$set:{
