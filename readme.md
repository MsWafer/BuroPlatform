...
- **PUT /users/reg2** - **_требует_** _name_, _lastname_, _position_, _division_(айди существующего отдела) в бади, опционально - аватарка
...
- **PUT /users/me** - меняет _name_,_email_,_position_ текущего пользователя на новые, взятые из body. Если они не указаны оставляет старые значения
